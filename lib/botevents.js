...
class BotEvents extends MusicBot {
    ...
    StartListeners() {
        this.client.on('ready', () => {
            console.log('Setup took: ' + (new Date - this.startTime) + 'ms');
    ...
    CheckChoice(msgReaction, userObj) {
        ...
        if (music.servers[serverId] != undefined && music.servers[serverId].choiceObj != undefined && music.servers[serverId].choiceObj[userId] != undefined) {
            ...
            function playSong(msgObj, song, exception) {
                ...
                if (music.hasQueue(serverId) == true) {
                    if (exception)
                        music.playNextInQueue(serverId, song, userId, () => {
                            return msgObj.channel.send('Playing song next up');
                        });
                    else
                        music.addToQueue(serverId, song, userId, () => {
                            msgObj.channel.send('Song has been added');
                            msgObj.delete(50);
                        });
                }
                else {
                    music.createQueue(serverId, song, msgObj.channel.id, userId, () => {
                        music.startQueue(msgObj.guild.id, choiceObj.voicechannel, () => {
                            msgObj.channel.send('Playback starting');
                            msgObj.delete(50);
                        });
                    });
                }
    ...
    HandleReaction(userObj, msgReaction, serverId, voicechannel) {
        ...
        if (emoji == '⏮') {
            HasCoolDown(userObj, 'previous', 5, bool => {
                if (bool == true) {
        ...
        if (emoji == '⏸') {
            HasCoolDown(userObj, 'pauseToggle', 5, bool => {
                if (bool == true) {
        ...
        if (emoji == '⏭') {
            HasCoolDown(userObj, 'skip', 5, bool => {
                if (bool == true) {
        ...
        if (emoji == '🔂') {
            HasCoolDown(userObj, 'repeat', 5, bool => {
                if (bool == true) {
    ...
    ReactionAction(msgReaction, userObj) {
        ...
        for (var i = 0; i < choiceEmojis.length; i++)
