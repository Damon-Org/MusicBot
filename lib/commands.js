...
function CheckFeatures(serverId, category, funCallback) {
    switch (category) {
        case 'music':
            global.conn.query(
                `SELECT value FROM settings WHERE guild_id = (SELECT guild_id FROM guilds WHERE serverId = '${serverId}') AND option_id = 1`,
...
function Volume(msgObj, context) {
    ...
    if (music.isDamonInVC(voicechannel)) {
        if (context[0] == '') {
            ...
            // if (!context[0].includes(',') && !isNaN(context[0]) && parseInt(context[0], 10) <= 5000 && parseInt(context[0], 10) >= 0) {
