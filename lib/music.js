...
class MusicSystem {
    ...
    createQueue(serverId, reqResponse, channel, userId, funCallback) {
        ...
        const base = {queue: [], volume: 0.15, playing: false, recording: false, repeat: 0, timer: null, stream: null, paused: false, channel: channel, msgObj: null};
    ...
    createStreamCountdown(serverId) {
        ...
        // Sets .playing to true to indicate that the current song still is playing
        ...
        this.servers[serverId].timer = new Timer(() => {
            // Timer has expired and we set the .playing method to false to indicate that a song should've ended
            this.servers[serverId].playing = false;
            // We call .end on our stream so the event listener knows a song has ended
    ...
    destroyQueue(serverId, funCallback) {
        this.disableMusicEmbed(serverId, () => {
            ...
            // if our timer isn't null yet it must still be running so we destroy that instance
        ...
        if (!this.servers[serverId].player && this.servers[serverId] != undefined) {
            ...
            voicechannel.join().then(conn => {
                ...
                audioStream.on('end', end => {
                    ...
                    else if (!this.servers[serverId].player) {
                        console.log("Something went wrong, attempting to fix...");

                        return this.playNext(serverId, voicechannel);
                    }

                    // Do nothing, still have to find a fix
                    return console.log('Song ended prematurely');
                });
            }).catch(err => {
                const
                    msgChannel = this.bot.channels.get(this.servers[serverId].channel),
