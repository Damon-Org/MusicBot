module.exports = class PlayNext {
    ...
    async onCommand(musicBot, msgObj, command, args) {
        ...
        switch (response) {
            ...
            default: {
                newMsg = await msgObj.reply('searching is not yet supported.');
        ...
        const music = (musicBot.serverUtils.getClassInstance(msgObj.guild.id)).musicSystem;

        song.setRequester(serverMember);

        if (song.list != null) {
            music.playlistEmbed(msgObj, voiceChannel, song);
            return;
        }

        if (music.queueExists()) {
            if (music.isDamonInVC(voiceChannel)) {
                await music.addToQueue(song, true);
                msgObj.channel.send(`Added song next up **${song.getTitle()}**`);

                return;
            }

            msgObj.reply(`you aren't in the bot's channel.`);

            return;
        }

        await music.createQueue(song, msgObj.channel);
        music.startQueue(voiceChannel);

        msgObj.channel.send(`No queue exists, playback starting with **${song.getTitle()}**`);
    }
}
