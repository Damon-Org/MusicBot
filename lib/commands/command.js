...
module.exports = class CommandRegisterer {
            ...
            if (this.config.hasOwnProperty(commandCategory)) {
                ...
                for (let i = 0; i < commandArr.length; i++) {
                    ...
                    try {
                        ...
                        console.log(`[COMMAND] Invalid instance or disabled: ${command.name}`);
                    }
                }
            }
        }

        console.log(`[COMMAND] Mapping of commands done with ${this.commands.size} commands registered.`);
    }
    ...
    async checkMessage(message) {
        ...
        if (content.startsWith(prefix)) {
            ...
            if (this.commands.has(command)) {
                ...
                if (! await this.beforeRunCommand(command, cmdObj, message, args)) {
                    return false;
                }
    ...
    checkParams(msgObj, cmdInstance, args) {
        const newEmbed = new this.musicBot.Discord.RichEmbed();
        let err = false;
