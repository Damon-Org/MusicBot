...
module.exports = class MusicSystem {
    ...
    async onChoiceEmbedAction(index, msgObj, user) {
        ...
        if (this.queueExists()) {
            ...
            await this.addToQueue(song, choiceInstance.shouldPlayNext);
        ...
        await this.createQueue(song, msgObj.channel);
    ...
    pausePlayback() {
        if (!this.paused) {
            ...
            let
                color = '#dd153d',
                footer = 'Paused | Repeat: Off';

            if (this.queue.active() == null) {
                footer = 'Paused | Song has been removed and repeat has been disabled';
            }
            else if ((this.queue.active()).repeat) {
                footer = 'Paused | Repeat: On';
            }
